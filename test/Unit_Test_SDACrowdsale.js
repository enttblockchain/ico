...
contract('LGTCrowdsale', function (accounts) {
    ...
    it('UNIT TESTS - LGTCrowdsale - Test Case 01: Cannot deploy LGTCrowdsale with incorrect variables', async function () {
        ...
        await LGTCrowdsale.new(expiredStart, sEndTime, saleCap, acctEight, tokenInst.address, oraclizeInst.address, {from: acctFour}).should.be.rejectedWith(EVMRevert);

        // Should fail to create Crowdsale with endTime before startTime
        await LGTCrowdsale.new(sEndTime, sStartTime, saleCap, acctEight, tokenInst.address, oraclizeInst.address, {from: acctFour}).should.be.rejectedWith(EVMRevert);

        // Should fail to create Crowdsale with goal higher than cap
        // await LGTCrowdsale.new(sStartTime, sEndTime, saleCap, saleGoal, acctEight, {from: acctFour}).should.be.rejectedWith(EVMRevert);

        // Should fail to create Crowdsale with blank/invalid wallet address (0x0)
        await LGTCrowdsale.new(sStartTime, sEndTime, saleCap, invalidAcct, tokenInst.address, oraclizeInst.address, {from: acctFour}).should.be.rejectedWith(EVMRevert);

        // Should fail to create Crowdsale with blank/invalid token address (0x0)
        await LGTCrowdsale.new(sStartTime, sEndTime, saleCap, acctEight, invalidAddr, oraclizeInst.address, {from: acctFour}).should.be.rejectedWith(EVMRevert);

        // Should fail to create Crowdsale with blank/invalid oraclize contract address (0x0)
        await LGTCrowdsale.new(sStartTime, sEndTime, saleCap, acctEight, tokenInst.address, invalidAddr, {from: acctFour}).should.be.rejectedWith(EVMRevert);
    }).timeout(timeoutDuration);

    it('UNIT TESTS - LGTCrowdsale - Test Case 02: LGTCrowdsale is deployed', async function () {
        currentTime = web3.eth.getBlock('latest').timestamp;
        ...
        assert.isNotNull(sInst,
            '\n     ' +
            'UNIT TESTS - LGTCROWDSALE - TEST CASE 02: Test #1\n      ' +
            'TEST DESCRIPTION: LGTCrowsale not initialized\n      ' +
        ...
        assert.equal((web3.fromWei(initialTokenSupply, 'ether')).toNumber(), tokenTotalSupply.toNumber(),
            '\n     ' +
            'UNIT TESTS - LGTCROWDSALE - TEST CASE 03: Test #2\n      ' +
            'TEST DESCRIPTION: LGTToken initial token supply\n      ' +
            'EXPECTED RESULT: ' + tokenTotalSupply + '\n      ' +
            'ACTUAL RESULT: ' + web3.fromWei(initialTokenSupply, 'ether'));
    }).timeout(timeoutDuration);

    it('UNIT TESTS - LGTCrowdsale - Test Case 03: Set WhiteList (Failed Cases)', async function() {
    ...
    it('UNIT TESTS - LGTCrowdsale - Test Case 04: Set WhiteList (Success Cases)', async function() {
    ...
    it('UNIT TESTS - LGTCrowdsale - Test Case 05: Cannot call calculatePrice with incorrect variables.', async function() {
        let zeroEth = 0;
    ...
    it('UNIT TESTS - LGTCrowdsale - Test Case 06: Call calculatePrice with correct variables', async function() {
        for (let index = 0; index < tokenSaleTier; index++) {
            const element = tokenSalePrice[index];
            ...
            assert.equal(actualTokenAmountCaseOne, element.expectedTokenAmount.toString(),
                '\n     ' +
                'UNIT TESTS - LGTCROWDSALE - TEST CASE 06: Test #' + index + '\n      ' +
    ...
    it('UNIT TESTS - LGTCrowdsale - Test Case 07: Call calculatePrice with correct variables (Test decimals points)', async function() {
        for (let i = 0; i < tokenSaleTier; i++) {
            ...
            assert.equal(actualTokenAmountCaseTwo.toFixed(3), elementCaseTwo.expectedTokenAmount_case_1.toFixed(3),
                '\n     ' +
                'UNIT TESTS - LGTCROWDSALE - TEST CASE 07: Test #' + i + '\n      ' +
    ...
    it('UNIT TESTS - LGTCrowdsale - Test Case 09: Buy Token (Failed Cases)', async function() {
    ...
    it('UNIT TESTS - LGTCrowdsale - Test Case 10: Buy Token (Success Cases)', async function() {
        let sendEther = web3.toWei(20, 'ether');
        let etherPrice;
        ...
        assert.equal(sendEther, logResult._value.toNumber(),
            '\n     ' +
            'UNIT TESTS - LGTCROWDSALE - TEST CASE 10: Shoud Log 2 ether after purchase\n      ' +
     ...
     */
    // it('UNIT TESTS - LGTCrowdsale - Test Case 11: Oraclize Service', async function() {
    //     let before = Date.now();
    //     let etherPrice;
    ...
    it('UNIT TESTS - LGTCrowdsale - Test Case 11: Address Migration (Failed Cases)', async function() {
        ...
        // Set account five into whilist, but still fail because target migrate address already have lgttoken value;
    ...
    // it('UNIT TESTS - LGTCrowdsale - Test Case 12: Address Migration (Success Cases)', async function() {
    ...
    //                 '\n     ' +
    //                 'UNIT TESTS - LGTCROWDSALE - TEST CASE 12: Test #1 Event Log new address param should same with requested new address\n      ' +
    //                 'TEST DESCRIPTION: AddresscMigrations is called\n      ' +
    //                 'EXPECTED RESULT: ' + newWallet + '\n      ' +
    //                 'ACTUAL RESULT: is ' + logResult.newAddress);

    //             assert.equal(oldWalletToken[1].toNumber(), logResult.newLgtToken.toNumber(),
    //                 '\n     ' +
    //                 'UNIT TESTS - LGTCROWDSALE - TEST CASE 12: Test #2 New wallet token should same with old walet token\n      ' +
    //                 'TEST DESCRIPTION: AddressMigrations is called\n      ' +
    //                 'EXPECTED RESULT: ' + oldWalletToken[1].toNumber() + '\n      ' +
    //                 'ACTUAL RESULT: is ' + logResult.newLgtToken.toNumber());
    //         }
    //     });
    ...
    it('UNIT TESTS - LGTCrowdsale - Test Case 13: Buy Token Min/Max USD amount', async function() {
    ...
    // it('UNIT TESTS - LGTCrowdsale - Test Case 14: Distributes Token (Failed Cases)', async function() {
    ...
    // it('UNIT TESTS - LGTCrowdsale - Test Case 15: Buy Token (ethers exceed cap amount) (Failed Cases)', async function() {
    //     currentTime = web3.eth.getBlock('latest').timestamp;
    //     sStartTime = currentTime;
    //     sEndTime = sStartTime + oneMonth * 2;
    //     saleCap = new BigNumber(web3.toWei(10, 'ether'));
    //     sInst = await LGTCrowdsale.new(sStartTime, sEndTime, saleCap, acctEight, tokenInst.address, {from: acctFour});
    ...
    // ];
    //
    // it('UNIT TESTS - LGTCrowdsale - Test Case 15: Buy Token-Test Valid Purchase (ethers exceed cap amount) (Failed Cases)', async function() {
    //     const evtWatch = sInst.LogTokenPurchase();
    ...
    it('UNIT TESTS - EtherOraclizeService - Test Case 16: transferBalances', async function () {
        ...
        assert.equal(contractBalance.toNumber(), 0,
            '\n     ' +
            'UNIT TESTS - EtherOraclizeService - TEST CASE 16: Contract balance should be 0\n      ' +
        ...
        assert.equal(contractBalance.toNumber(), twoEther,
            '\n     ' +
            'UNIT TESTS - EtherOraclizeService - TEST CASE 16: Contract balance should be 2 ether\n      ' +
            'TEST DESCRIPTION: Buy Token is called\n      ' +
